...
dark_cum_std = np.nanstd(np.array(interpolated_cums_dark), axis=0)

light_cum_means = np.nanmean(np.array(interpolated_cums_light), axis=0)
light_cum_std = np.nanstd(np.array(interpolated_cums_light), axis=0)

normalised_dark_cum_means = np.nanmean(np.array(normalised_interpolated_dark), axis=0)
normalised_dark_cum_std = np.nanstd(np.array(normalised_interpolated_dark), axis=0)

normalised_light_cum_means = np.nanmean(np.array(normalised_interpolated_light), axis=0)
normalised_light_cum_std = np.nanstd(np.array(normalised_interpolated_light), axis=0)
...
ax[0].fill_between(x_plot, dark_cum_means+dark_cum_std, dark_cum_means-dark_cum_std,
                  color='magenta', alpha=0.3)

ax[0].plot(x_plot, light_cum_means,
          color='blue', linewidth= 3.0, label='Light')
ax[0].fill_between(x_plot, light_cum_means+light_cum_std, light_cum_means-light_cum_std,
...
ax[1].plot(x_plot, normalised_light_cum_means,
          color='blue', linewidth= 3.0, label='Light')
ax[1].fill_between(x_plot, normalised_light_cum_means+normalised_light_cum_std,
                   normalised_light_cum_means-normalised_light_cum_std,
                  color='blue', alpha=0.3)
